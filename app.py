...
def reorder_conditions(clinic_name: str, move_id: str, insert_before_id):
    """move_id の条件を insert_before_id の直前に移動。None なら末尾。"""
    conditions = list(st.session_state["all_conditions"][clinic_name])
    item = next(c for c in conditions if c["id"] == move_id)
    rest = [c for c in conditions if c["id"] != move_id]
    if insert_before_id is None:
        new_order = rest + [item]
    else:
        idx = next(i for i, c in enumerate(rest) if c["id"] == insert_before_id)
        new_order = rest[:idx] + [item] + rest[idx:]
    sb = get_supabase()
    for i, cond in enumerate(new_order, start=1):
        cond["sort_order"] = i
        sb.table("conditions").update({"sort_order": i}).eq("id", cond["id"]).execute()
    st.session_state["all_conditions"][clinic_name] = new_order
...
if "reorder_mode" not in st.session_state:
    st.session_state["reorder_mode"] = {}  # { clinic_name: bool }
if "custom_cond_order" not in st.session_state:
    st.session_state["custom_cond_order"] = {}  # { clinic_name: [name, ...] }
...
with tab2:
    ...
    if not conditions:
        ...
        # 入力用DataFrameを作成（保存済みがあれば復元、なければ空行）
        ...
        load_all  = c2.button("マスタ全件を読み込む", key="load_all_conds", use_container_width=True)

        if load_all:
            st.session_state["custom_cond_order"][selected_clinic] = [c["name"] for c in conditions]
            st.rerun()

        if run_custom:
            pasted = [str(n).strip() for n in edited_df["施術名"].tolist() if str(n).strip()]
            not_found = [n for n in pasted if n not in name_to_cond]
            if not_found:
                st.warning(f"条件マスタに見つかりません: {', '.join(not_found)}")
            pasted = [n for n in pasted if n in name_to_cond]
            if pasted:
                st.session_state["custom_cond_order"][selected_clinic] = pasted
            else:
...
with tab4:
    ...
    if not conditions:
        ...
        with btn_cols[1]:
            if st.button(f"🗑 選択削除（{len(selected)}件）", key="sel_del",
                ...
                st.rerun()
        with btn_cols[2]:
            reorder_mode = st.session_state["reorder_mode"].get(clinic_name, False)
            label = "✅ 並び替え中" if reorder_mode else "🔀 並び替え"
            if st.button(label, key="reorder_btn", use_container_width=True):
                st.session_state["reorder_mode"][clinic_name] = not reorder_mode
                st.rerun()
        with btn_cols[3]:
            bulk_step = st.session_state["bulk_delete_step"].get(clinic_name, 0)
            if bulk_step == 0:
                if st.button("🗑 全件削除", key="bulk_del", use_container_width=True):
                    st.session_state["bulk_delete_step"][clinic_name] = 1
                    st.rerun()
            elif bulk_step == 1:
                st.warning(f"**{clinic_name}** の条件を全件削除します。よいですか？")
                y1, n1 = st.columns(2)
                if y1.button("はい", key="bulk_y1", use_container_width=True):
                    st.session_state["bulk_delete_step"][clinic_name] = 2
                    st.rerun()
                if n1.button("いいえ", key="bulk_n1", use_container_width=True):
                    st.session_state["bulk_delete_step"][clinic_name] = 0
                    st.rerun()
            elif bulk_step == 2:
                st.error("最終確認：この操作は取り消せません。")
                y2, n2 = st.columns(2)
                if y2.button("削除する", key="bulk_y2", use_container_width=True):
                    delete_all_conditions(clinic_name)
                    st.session_state["all_conditions"][clinic_name] = []
                    st.session_state["bulk_delete_step"][clinic_name] = 0
                    st.session_state["selected_ids"][clinic_name] = set()
                    st.rerun()
                if n2.button("キャンセル", key="bulk_n2", use_container_width=True):
                    st.session_state["bulk_delete_step"][clinic_name] = 0
                    st.rerun()

        # ── 並び替えモード ──
        if st.session_state["reorder_mode"].get(clinic_name, False):
            st.info("移動する条件と挿入先を選んで「実行」してください。")
            names = [c["name"] for c in conditions]
            move_name = st.selectbox("移動する条件", names, key="reorder_move")
            move_id = next(c["id"] for c in conditions if c["name"] == move_name)
            remaining = [c for c in conditions if c["id"] != move_id]
            insert_options = [f"「{c['name']}」の前" for c in remaining] + ["最後に追加"]
            insert_choice = st.selectbox("挿入先", insert_options, key="reorder_insert")
            r1, r2 = st.columns(2)
            if r1.button("実行", key="reorder_exec", use_container_width=True):
                if insert_choice == "最後に追加":
                    insert_before_id = None
                else:
                    idx = insert_options.index(insert_choice)
                    insert_before_id = remaining[idx]["id"]
                reorder_conditions(clinic_name, move_id, insert_before_id)
                st.session_state["reorder_mode"][clinic_name] = False
                st.rerun()
            if r2.button("キャンセル", key="reorder_cancel", use_container_width=True):
                st.session_state["reorder_mode"][clinic_name] = False
                st.rerun()
            ...
            for cond in filtered_conds:
                ...
                if st.session_state["edit_id"] == cid:
                    with st.form(key=f"edit_form_{cid}"):
                        ...
                        no_campaign = st.checkbox("キャンペーン終了日なし", value=(current_end is None))
                        try:
                            default_end = datetime.date.fromisoformat(str(current_end)) if current_end else datetime.date.today()
                        except Exception:
                            default_end = datetime.date.today()
                        new_end = st.date_input("キャンペーン終了日", value=default_end)
                        sb, cb = st.columns(2)
                    ...
                    if submitted:
                        if not new_name.strip():
                            ...
                            updated = {
                                ...
                                "campaign_end": None if no_campaign else new_end.isoformat(),
                            }
    ...
    with st.form(f"add_form_{clinic_name}", clear_on_submit=True):
        ...
        has_campaign_add = st.checkbox("キャンペーン終了日を設定する")
        add_end   = st.date_input("キャンペーン終了日", value=datetime.date.today())
    ...
    if add_submit:
        if not add_name.strip():
            ...
            new_cond = {
                ...
                "campaign_end": add_end.isoformat() if has_campaign_add else None,
            }
