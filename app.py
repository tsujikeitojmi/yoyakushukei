...
def swap_sort_order(id1: str, order1: int, id2: str, order2: int):
    sb = get_supabase()
    sb.table("conditions").update({"sort_order": order2}).eq("id", id1).execute()
    sb.table("conditions").update({"sort_order": order1}).eq("id", id2).execute()
...
# ==========================================================================
with tab1:
    base_result   = aggregate_base(range_detail_df)
    base_transposed = aggregate_base_transposed(range_detail_df)
...
with tab2:
    ...
    if not conditions:
        ...
        cond_result = aggregate_conditions_transposed(range_after_cancel_df, conditions)
    ...
    for clinic_tab, clinic_name in zip(clinic_tabs, CLINICS):
        with clinic_tab:
            ...
            if not conditions:
                ...
                h = st.columns([2, 3, 3, 0.5, 0.5, 0.8, 0.8])
                ...
                for cond in filtered_conds:
                    ...
                    if st.session_state["edit_id"] == cid:
                        ...
                        idx = next(i for i, c in enumerate(conditions) if c["id"] == cid)
                        row_cols = st.columns([2, 3, 3, 0.5, 0.5, 0.8, 0.8])
                        row_cols[0].write(cond["name"])
                        row_cols[1].write(cond.get("category",  "") or "（指定なし）")
                        row_cols[2].write(cond.get("treatment", "") or "（指定なし）")
                        if row_cols[3].button("▲", key=f"up_{clinic_name}_{cid}",
                                              disabled=(idx == 0)):
                            a, b = conditions[idx], conditions[idx - 1]
                            swap_sort_order(a["id"], a["sort_order"], b["id"], b["sort_order"])
                            a["sort_order"], b["sort_order"] = b["sort_order"], a["sort_order"]
                            conditions[idx], conditions[idx - 1] = b, a
                            st.session_state["all_conditions"][clinic_name] = conditions
                            st.rerun()
                        if row_cols[4].button("▼", key=f"down_{clinic_name}_{cid}",
                                              disabled=(idx == len(conditions) - 1)):
                            a, b = conditions[idx], conditions[idx + 1]
                            swap_sort_order(a["id"], a["sort_order"], b["id"], b["sort_order"])
                            a["sort_order"], b["sort_order"] = b["sort_order"], a["sort_order"]
                            conditions[idx], conditions[idx + 1] = b, a
                        ...
                        if row_cols[5].button("編集", key=f"edit_btn_{clinic_name}_{cid}"):
                            ...
                            st.rerun()
                        if row_cols[6].button("削除", key=f"del_btn_{clinic_name}_{cid}"):
            ...
            with st.expander("🔍 条件テスト（追加前に確認）"):
                ...
                prev_cat   = st.text_input("施術カテゴリー条件（テスト）",
                                            placeholder="空白=すべて対象",
                                            key=f"prev_cat_{clinic_name}")
                prev_treat = st.text_input("施術名条件（テスト）",
                                            placeholder="空白=すべて対象",
                                            key=f"prev_treat_{clinic_name}")
