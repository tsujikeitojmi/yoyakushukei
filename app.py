...
def sticky_table_html(df: pd.DataFrame) -> str:
    """1列目を固定した横スクロールHTMLテーブルを返す"""
    import html as _html
    bg = "#ffffff"
    header_bg = "#f5f5f5"
    border = "1px solid #e0e0e0"
    sticky_style = (
        f"position:sticky;left:0;z-index:2;background:{bg};"
        "white-space:nowrap;padding:6px 10px;border:" + border + ";"
    )
    sticky_header_style = (
        f"position:sticky;left:0;z-index:3;background:{header_bg};"
        "white-space:nowrap;padding:6px 10px;border:" + border + ";"
        "font-weight:600;color:#C04F6A;font-size:13px;"
    )
    cell_style = f"padding:6px 10px;border:{border};white-space:nowrap;text-align:right;"
    header_cell_style = (
        f"padding:6px 10px;border:{border};white-space:nowrap;"
        "font-weight:600;color:#C04F6A;font-size:13px;background:{header_bg};text-align:center;"
    )
    ...
    for i, col in enumerate(df.columns):
        s = sticky_header_style if i == 0 else header_cell_style
        rows_html += f"<th style='{s}'>{_html.escape(str(col))}</th>"
    rows_html += "</tr>"
    # body
    for _, row in df.iterrows():
        ...
        for i, col in enumerate(df.columns):
            val = row[col]
            if i == 0:
                rows_html += f"<td style='{sticky_style}'>{_html.escape(str(val))}</td>"
            else:
                display_val = "" if val == "" else _html.escape(str(val))
                rows_html += f"<td style='{cell_style}'>{display_val}</td>"
        rows_html += "</tr>"
...
with tab1:
    ...
    st.markdown(sticky_table_html(base_transposed), unsafe_allow_html=True)
    st.download_button(
        "📥 予約件数をCSVで保存",
...
with tab2:
    ...
    if not conditions:
        ...
        if not ordered_conds:
            ...
            st.markdown(sticky_table_html(display2), unsafe_allow_html=True)
            st.download_button(
                "📥 集計施術別をCSVで保存",
