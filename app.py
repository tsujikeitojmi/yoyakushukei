...
def render_sticky_table(df: pd.DataFrame):
    """1列目を固定・ドラッグ選択＋TSVコピー対応テーブルをiframeで描画"""
    ...
    for i, col in enumerate(df.columns):
        if i == 0:
            rows_html += f"<th class='sticky-col'>{_html.escape(str(col))}</th>"
        else:
            rows_html += f"<th>{_html.escape(str(col))}</th>"
    rows_html += "</tr>"
    ...
    for _, row in df.iterrows():
        ...
        for i, col in enumerate(df.columns):
            ...
            if i == 0:
                rows_html += f"<td class='sticky-col'>{display_val}</td>"
            else:
                rows_html += f"<td>{display_val}</td>"
        rows_html += "</tr>"

    row_count = len(df) + 1
    height = row_count * 34 + 40
...
<style>
  ...
  th {{ background:#f5f5f5; color:#C04F6A; font-weight:600; text-align:center; }}
  td {{ text-align:right; }}
  .sticky-col {{ position:sticky; left:0; z-index:2; background:#ffffff; text-align:left; }}
  th.sticky-col {{ background:#f5f5f5; z-index:3; }}
...
document.addEventListener('copy', function(e) {{
  ...
  document.querySelectorAll('td').forEach(function(td) {{
    if (sel.containsNode(td, true)) {{
      var r = td.parentElement.rowIndex;
      var c = td.cellIndex;
      if (!rows[r]) rows[r] = {{}};
      rows[r][c] = td.innerText.trim();
    }}
  }});
  var rKeys = Object.keys(rows).sort(function(a,b){{return a-b;}});
  if (!rKeys.length) return;
  var tsv = rKeys.map(function(r) {{
    var cKeys = Object.keys(rows[r]).sort(function(a,b){{return a-b;}});
    return cKeys.map(function(c){{ return rows[r][c]; }}).join('\\t');
  }}).join('\\n');
...
with tab1:
    ...
    render_sticky_table(base_transposed)
    st.download_button(
        "📥 予約件数をCSVで保存",
